...
TEST(map_public_test) {
  ...
  /*
  //Custom test starts here
  ASSERT_TRUE(words.size() == 1);

  //cout << words["hello"] << endl;

  words["age"] = 19;
  ASSERT_TRUE(words.find("a") == words.end());
  ASSERT_TRUE(words.find("age")->first == "age");

  cout << words["age"] << endl;
  ASSERT_TRUE(words.find("age")->second == 19);


  ASSERT_EQUAL(words["age"], 19);
  ...
  */
  ...
  ASSERT_TRUE(words.find("hello")->first == "hello");
  ASSERT_TRUE(words.find("world")->first == "world");
  ASSERT_TRUE(words.find("pi")->first == "pi");

  ASSERT_TRUE(words.find("hello")->second == 1);
  ASSERT_TRUE(words.find("world")->second == 2);
  ASSERT_TRUE(words.find("pi")->second == 3.14159);
