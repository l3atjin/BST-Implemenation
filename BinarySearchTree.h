...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  // return the node if u find it
  // connect it tiwtht he tree
  static Node * insert_impl(Node *node, const T &item, Compare less) {
	  ...
	  else if (!node->right && !less(item, node->datum))
	  {
		  Node* temp = new Node;
		  node->right = temp;
		  temp->datum = item;
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
	  ...
	  else if (less(val, node->datum) && !node->left)
	  {
		  return node;
	  }
