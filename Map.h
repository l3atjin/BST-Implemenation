...
private:
  ...
  public:
	  ...
	  {
		  Key_type aKey = a.first;
		  Key_type bKey = b.first;
...
public:
  ...
  Value_type& operator[](const Key_type& k)
  {
	  ...
	  std::pair<Iterator, bool> temp1 = this->insert(temp);
	  //Pair_type temp3 = temp1.second;
	  Iterator temp2 = temp1.first;

	  Value_type& value = temp2->second;
	  return value;

	  /*Iterator entry = this->find(k);
	  if (entry != this->end())
	  {
  ...
  {
	  auto entry = this->find(val.first);
	  if (entry != this->end())
	  {
		  std::pair<Iterator, bool> temp = { entry, false };
		  return temp;
	  }
	  entries.insert(val);
	  std::pair<Iterator, bool> temp = { this->find(val.first), true };
	  return temp;
  }
